...
The first step would be to [upload your SSH Key](../developer-guide/ssh-key.html) to authorise your deployment via git-push process. Please note that **failure to provide (upload) a unique SSH key would result in error "Permission denied" during the code Push phase**.
